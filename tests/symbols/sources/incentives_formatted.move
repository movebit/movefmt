...
module econia::incentives{
  ...
  fun get_cost_to_upgrade_integrator_fee_store_view<QuoteCoinType, UtilityCoinType>(
    ...
    // coin type.
    let integrator_fee_stores_map_ref = &borrow_global<IntegratorFeeStores<QuoteCoinType>>(integrator_address).
     map;
  ...
  public fun get_integrator_withdrawal_fee<QuoteCoinType>(integrator: &signer, market_id: u64,): u64 acquires IncentiveParameters, IntegratorFeeStores{
  ...
  public fun upgrade_integrator_fee_store<QuoteCoinType, UtilityCoinType>(
    ...
    let cost = get_cost_to_upgrade_integrator_fee_store<QuoteCoinType, UtilityCoinType>(integrator, market_id, new_tier);
    ...
    // Borrow mutable reference to integrator fee stores map for
    // quote coin type.
    let integrator_fee_stores_map_ref_mut = &mut borrow_global_mut<IntegratorFeeStores<QuoteCoinType>>(integrator_address).
     map;
    // Borrow mutable reference to integrator fee store for given
    // market ID.
  ...
  public fun withdraw_econia_fees_all<QuoteCoinType>(econia: &signer, market_id: u64,): coin::
   Coin<QuoteCoinType> acquires EconiaFeeStore{
  ...
  public fun withdraw_integrator_fees<QuoteCoinType, UtilityCoinType>(
    ...
    // Borrow mutable reference to integrator fee stores map for
    // quote coin type.
    let integrator_fee_stores_map_ref_mut = &mut borrow_global_mut<IntegratorFeeStores<QuoteCoinType>>(address_of(integrator)).
     map;
    // Borrow mutable reference to integrator fee store for given
    // market ID.
    let integrator_fee_store_ref_mut = tablist::borrow_mut(
    ...
    let withdrawal_fee = get_tier_withdrawal_fee(integrator_fee_store_ref_mut.
       tier);
    // Deposit verified amount and type of utility coins.
  ...
  public fun withdraw_utility_coins<UtilityCoinType>(econia: &signer, amount: u64): coin::
   Coin<UtilityCoinType> acquires UtilityCoinStore{
  ...
  public fun withdraw_utility_coins_all<UtilityCoinType>(econia: &signer): coin::
   Coin<UtilityCoinType> acquires UtilityCoinStore{
  ...
  public entry fun withdraw_econia_fees_all_to_coin_store<QuoteCoinType>(econia: &signer, market_id: u64,) acquires EconiaFeeStore{
    withdraw_econia_fees_to_coin_store_internal<QuoteCoinType>(econia, market_id, true,0);
  } 
  ...
  public entry fun withdraw_econia_fees_to_coin_store<QuoteCoinType>(
    ...
    withdraw_econia_fees_to_coin_store_internal<QuoteCoinType>(econia, market_id, false, amount);
  } 
  ...
  ///
  /// * `test_register_assess_withdraw()`
  public entry fun withdraw_integrator_fees_via_coinstores<QuoteCoinType, UtilityCoinType>(integrator: &signer, market_id: u64) acquires IncentiveParameters, IntegratorFeeStores, UtilityCoinStore{
    // Get fee to withdraw from integrator fee coin store.
    let withdrawal_fee = get_integrator_withdrawal_fee<QuoteCoinType>(integrator, market_id);
    ...
    withdraw_integrator_fees<QuoteCoinType, UtilityCoinType>(integrator, market_id, utility_coins);
    // Get integrator address.
    let integrator_address = address_of(integrator);
    // If integrator does not have quote coin store, register one.
    if(! coin::is_account_registered<QuoteCoinType>(integrator_address)) coin::
     register<QuoteCoinType>(integrator);
  ...
  ///
  /// * `test_withdraw_to_coin_store_econia()`
  public entry fun withdraw_utility_coins_all_to_coin_store<UtilityCoinType>(econia: &signer,) acquires UtilityCoinStore{
  ...
  ///
  /// * `test_withdraw_to_coin_store_econia()`
  public entry fun withdraw_utility_coins_to_coin_store<UtilityCoinType>(econia: &signer, amount: u64) acquires UtilityCoinStore{
  ...
  public(friend) fun assess_taker_fees<QuoteCoinType>(
    ...
    // quote coin type.
    let econia_fee_store_map_ref_mut = &mut borrow_global_mut<EconiaFeeStore<QuoteCoinType>>(fee_account_address).
     map;
  ...
  public(friend) fun calculate_max_quote_match(
    ...
    let denominator = if(direction == BUY)(taker_fee_divisor + 1 as u128) else(taker_fee_divisor -
       1 as u128);
  ...
  public(friend) fun deposit_custodian_registration_utility_coins<UtilityCoinType>(coins: coin::
     Coin<UtilityCoinType>) acquires IncentiveParameters, UtilityCoinStore{
  ...
  ///
  /// * `test_deposit_registration_fees_mixed()`
  public(friend) fun deposit_market_registration_utility_coins<UtilityCoinType>(coins: coin::
     Coin<UtilityCoinType>) acquires IncentiveParameters, UtilityCoinStore{
  ...
  ///
  /// * `test_deposit_registration_fees_mixed()`
  public(friend) fun deposit_underwriter_registration_utility_coins<UtilityCoinType>(coins: coin::
     Coin<UtilityCoinType>) acquires IncentiveParameters, UtilityCoinStore{
    deposit_utility_coins_verified<UtilityCoinType>(
      coins,
      get_underwriter_registration_fee()
  ...
  ///
  /// * `test_register_assess_withdraw()`
  public(friend) fun register_econia_fee_store_entry<QuoteCoinType>(market_id: u64) acquires EconiaFeeStore,{
    ...
    if(! exists<EconiaFeeStore<QuoteCoinType>>(fee_account_address)) // Move to the Econia fee account an empty one.
  ...
  public(friend) fun register_integrator_fee_store<QuoteCoinType, UtilityCoinType>(
    ...
    if(! exists<IntegratorFeeStores<QuoteCoinType>>(integrator_address)) // Move to the integrator account an empty one.
  ...
  ///
  /// * `test_deposit_withdraw_utility_coins()`
  fun deposit_utility_coins<UtilityCoinType>(coins: coin::Coin<UtilityCoinType>) acquires UtilityCoinStore{
    // Get fee account address.
    let fee_account_address = resource_account::get_address();
    // Borrow mutable reference to coins in utility coin store.
    let utility_coins_ref_mut = &mut borrow_global_mut<UtilityCoinStore<UtilityCoinType>>(fee_account_address).
     coins;
  ...
  fun init_utility_coin_store<CoinType>(fee_account: &signer){
    ...
    if(! exists<UtilityCoinStore<CoinType>>(address_of(fee_account))) // Move to the fee account an initialized one.
  ...
  fun set_incentive_parameters_parse_tiers_vector(
    ...
    while(i < n_tiers){// Loop over all specified tiers
      ...
      if(i == 0){// If parsing parameters for first tier:
        ...
        );// If parameters for tier that is not first:
      } else{
  ...
  fun withdraw_econia_fees_to_coin_store_internal<QuoteCoinType>(
    ...
    let coins = withdraw_econia_fees_internal<QuoteCoinType>(econia, market_id, all, amount);
    // If Econia does not have coin store for coin type:
    if(! coin::is_account_registered<QuoteCoinType>(@econia)) // Register one.
  ...
  ///
  /// * `E_NOT_ECONIA`: `account` is not Econia account.
  fun withdraw_utility_coins_internal<UtilityCoinType>(account: &signer, all: bool, amount: u64): coin::
   Coin<UtilityCoinType> acquires UtilityCoinStore{
    // Assert account is Econia.
    assert!(
      address_of(account) == @econia,
  ...
  ///
  /// * `test_withdraw_to_coin_store_econia()`
  fun withdraw_utility_coins_to_coin_store_internal<UtilityCoinType>(econia: &signer, all: bool, amount: u64) acquires UtilityCoinStore{
    // Withdraw coins from fee store, verifying Econia signer.
    let coins = withdraw_utility_coins_internal<UtilityCoinType>(econia, all, amount);
    // If Econia does not have coin store for coin type:
    if(! coin::is_account_registered<UtilityCoinType>(@econia)) // Register one.
  ...
  public fun get_econia_fee_store_balance_test<QuoteCoinType>(market_id: u64): u64 acquires EconiaFeeStore{
    coin::value(tablist::borrow(
        &borrow_global<EconiaFeeStore<QuoteCoinType>>(resource_account::get_address()).
         map,
        market_id
      ))
  }
  ...
  /// * Restricted to test-only to prevent excessive public queries
  ///   and thus transaction collisions.
  public fun get_integrator_fee_store_balance_test<QuoteCoinType>(integrator: address, market_id: u64): u64 acquires IntegratorFeeStores{
    coin::value(&tablist::borrow(
  ...
  /// * Restricted to test-only to prevent excessive public queries
  ///   and thus transaction collisions.
  public fun get_integrator_fee_store_tier_test<QuoteCoinType>(integrator: address, market_id: u64): u8 acquires IntegratorFeeStores{
    tablist::borrow(
      &borrow_global<IntegratorFeeStores<QuoteCoinType>>(integrator).map,
      market_id
    ).tier
  ...
  public fun get_utility_coin_store_balance_test(): u64 acquires UtilityCoinStore{
    coin::value(&borrow_global<UtilityCoinStore<UC>>(resource_account::get_address()).
       coins)
  }
  ...
  public fun init_test() acquires IncentiveParameters{
    ...
    let econia = account::create_signer_with_capability(&account::create_test_signer_cap(@econia));
  ...
  /// * Restricted to test-only to prevent excessive public queries
  ///   and thus transaction collisions.
  public fun has_integrator_fee_store_test<QuoteCoinType>(integrator: address, market_id: u64): bool acquires IntegratorFeeStores{
    ...
    let integrator_fee_stores_map_ref = &borrow_global<IntegratorFeeStores<QuoteCoinType>>(integrator).
     map;
  ...
  fun test_calculate_max_quote_match_overflow(){
    ...
    let max_quote_match_expected = ((taker_fee_divisor as u128)*((max_quote_delta_user -
           1) as u128))/((taker_fee_divisor - 1) as u128);
  ...
  fun test_deposit_registration_fees_mixed() acquires IncentiveParameters, UtilityCoinStore{
    ...
    // Deposit fees.
    deposit_market_registration_utility_coins<UC>(assets::mint_test(market_registration_fee));
    deposit_underwriter_registration_utility_coins<UC>(assets::mint_test(underwriter_registration_fee));
    deposit_custodian_registration_utility_coins<UC>(assets::mint_test(custodian_registration_fee));
    // Assert total amount.
    assert!(
  ...
  fun test_get_cost_to_upgrade_integrator_fee_store_not_increase(integrator: &signer) acquires IncentiveParameters, IntegratorFeeStores, UtilityCoinStore{
    init_test();// Init incentives.
    ...
    let integrator_fee_store_tiers_ref_mut = &mut incentive_parameters_ref_mut.
     integrator_fee_store_tiers;
  ...
  fun test_get_cost_to_upgrade_integrator_fee_store_not_upgrade(integrator: &signer) acquires IncentiveParameters, IntegratorFeeStores, UtilityCoinStore{
    init_test();// Init incentives.
  ...
  fun test_register_assess_withdraw(econia: &signer, integrator: &signer) acquires EconiaFeeStore, IncentiveParameters, IntegratorFeeStores, UtilityCoinStore{
    ...
    let utility_coin_balance_0 = get_tier_activation_fee(tier_0) + get_tier_activation_fee(tier_1);
    // Declare utility coin balance after integrator fee withdrawal.
    let utility_coin_balance_1 = utility_coin_balance_0 + get_tier_withdrawal_fee(tier_0);
    ...
    assert!(
      get_econia_fee_store_balance_test<QC>(market_id_0) == econia_fees_0,// Assert Econia fee share.
      0
    );
    assert!(
      get_integrator_fee_store_balance_test<QC>(@user, market_id_0) == integrator_fees_0,// Assert integrator fee share.
      0
    );
    ...
    coin::destroy_zero(quote_coins);
    assert!(
      get_econia_fee_store_balance_test<QC>(market_id_1) == econia_fees_1,// Assert Econia fee share.
      0
    );
    ...
    coin::destroy_zero(quote_coins);
    assert!(
      get_econia_fee_store_balance_test<QC>(market_id_2) == econia_fees_2,// Assert Econia fee share.
      0
    );
  ...
  fun test_upgrade_integrator_fee_store_via_coinstore(integrator: &signer) acquires IncentiveParameters, IntegratorFeeStores, UtilityCoinStore{
    init_test();// Init incentives.
    ...
    upgrade_integrator_fee_store_via_coinstore<QC, UC>(integrator, market_id, tier_upgrade);
  ...
  fun test_withdraw_to_coin_store_econia() acquires EconiaFeeStore, IncentiveParameters, UtilityCoinStore{
    ...
    let econia_fee_store_map_ref_mut = &mut borrow_global_mut<EconiaFeeStore<QC>>(resource_account::
       get_address()).map;
    // Borrow mutable reference to fees for given market ID.
    let econia_fee_store_coins_ref_mut = tablist::borrow_mut(
      econia_fee_store_map_ref_mut,
