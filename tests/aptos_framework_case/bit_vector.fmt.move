module std::bit_vector {
        ...
        if (amount >= bitvector.length) {
            ...
            while ({
                    spec {
                        ...
                        invariant forall k in 0..bitvector.length - amount: bitvector.bit_field[k] == old(bitvector).bit_field[k
                            + amount];
    ...
    spec shift_left_for_verification_only {
        ...
        ensures amount < bitvector.length ==> (forall i in 0..bitvector.length - amount: bitvector.bit_field[i] ==
