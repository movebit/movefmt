...
module DiemFramework::DiemAccount {
    ...
    fun should_track_limits_for_account<Token>(
        ...
        if (is_withdrawal) {
            has_published_account_limits<Token>(payer) && VASP::is_vasp(payer) && !VASP::is_same_vasp(
                payer, payee)
        } else {
    ...
    spec fun spec_should_track_limits_for_account<Token>(payer: address, payee: address,
        is_withdrawal: bool): bool {
        if (is_withdrawal) {
            spec_has_published_account_limits<Token>(payer) && VASP::is_vasp(payer) && !VASP::spec_is_same_vasp(
                payer, payee)
        } else {
    ...
    spec schema DepositAbortsIfRestricted<Token> {
        ...
        aborts_if spec_should_track_limits_for_account<Token>(payer, payee, false) && !AccountLimits::spec_update_deposit_limits<Token>(
            amount, VASP::spec_parent_address(payee)) with errors::LIMIT_EXCEEDED;
    ...
    public fun tiered_mint<Token>(
        ...
        let coin =
            DesignatedDealer::tiered_mint<Token>(tc_account, mint_amount,
                designated_dealer_address, tier_index);
    ...
    spec tiered_mint {
        ...
        modifies global<AccountLimits::Window<Token>>(VASP::spec_parent_address(
    ...
    spec schema TieredMintEnsures<Token> {
        ...
        == update_field(currency_info, total_value, currency_info.total_value + mint_amount);
    ...
    fun withdraw_from<Token>(
        ...
        assert!(exists<Balance<Token>>(payer),
    ...
    public fun extract_withdraw_capability(sender: &signer): WithdrawCapability acquires DiemAccount {
        ...
        assert!(!delegated_withdraw_capability(sender_addr),
    ...
    spec extract_withdraw_capability {
        ...
        == update_field(old(global<DiemAccount>(sender_addr)), withdraw_capability,
            option::spec_none());
    ...
    public fun restore_withdraw_capability(cap: WithdrawCapability) acquires DiemAccount {
        ...
        assert!(delegated_withdraw_capability(cap.account_address),
    ...
    spec restore_withdraw_capability {
        ...
        == update_field(old(global<DiemAccount>(cap_addr)), withdraw_capability,
    ...
    public fun rotate_authentication_key(
        ...
        assert!(vector::length(&new_authentication_key) == 32,
    ...
    public fun extract_key_rotation_capability(account: &signer): KeyRotationCapability acquires DiemAccount {
        ...
        assert!(!delegated_key_rotation_capability(account_address),
    ...
    fun make_account(new_account: &signer, auth_key_prefix: vector<u8>,) acquires AccountOperationsCapability {
        ...
        assert!(new_account_addr != @VMReserved,
            errors::invalid_argument(ECANNOT_CREATE_AT_VM_RESERVED));
        assert!(new_account_addr != @DiemFramework,
        ...
        assert!(exists<AccountOperationsCapability>(@DiemRoot),
            errors::not_published(EACCOUNT_OPERATIONS_CAPABILITY));
        // Emit the CreateAccountEvent
        event::emit_event(&mut borrow_global_mut<AccountOperationsCapability>(@DiemRoot).creation_events,
            CreateAccountEvent {
        ...
        move_to(new_account,
            DiemAccount {
                ...
                key_rotation_capability: option::some(KeyRotationCapability {
                        account_address: new_account_addr
                    }),
    ...
    spec make_account {
        ...
        == update_field(account_ops_cap, creation_events, account_ops_cap.creation_events);
    ...
    spec schema MakeAccountAbortsIf {
        ...
        aborts_if DiemTimestamp::is_genesis() && !exists<AccountOperationsCapability>(@DiemRoot) with errors::NOT_PUBLISHED;
    ...
    fun create_authentication_key(
        ...
        vector::append(&mut authentication_key,
    ...
    fun create_diem_root_account(auth_key_prefix: vector<u8>,) acquires AccountOperationsCapability {
        ...
        assert!(!exists<DiemWriteSetManager>(@DiemRoot),
    ...
    public fun create_designated_dealer<CoinType>(
        ...
        DesignatedDealer::publish_designated_dealer_credential<CoinType>(&new_dd_account,
            creator_account, add_all_currencies);
    ...
    public fun create_parent_vasp_account<Token>(
        ...
        spec {
            ...
            assert Roles::spec_has_treasury_compliance_role_addr(signer::address_of(
                    creator_account
                ));
        }
    }
    ...
    public fun balance<Token>(addr: address): u64 acquires Balance {
        assert!(exists<Balance<Token>>(addr),
            errors::not_published(EPAYER_DOESNT_HOLD_CURRENCY));
    ...
    public(friend) fun add_currency<Token>(account: &signer) {
        ...
        assert!(Roles::can_hold_balance(account),
    ...
    spec schema AddCurrencyEnsures<Token> {
        ...
        ensures global<Balance<Token>>(addr) == Balance<Token> { coin: Diem<Token> { value: 0 } };
    }
    ...
    spec schema ScriptPrologueAbortsIf<Token> {
        ...
        include DiemTransactionPublishingOption::AbortsIfNoTransactionPublishingOption;
        /// Covered: L74 (Match 8)
        aborts_if !DiemTransactionPublishingOption::spec_is_script_allowed(sender,
            script_hash) with errors::INVALID_STATE;
    }
    ...
    fun writeset_prologue(
        ...
        assert!(signer::address_of(&sender) == @DiemRoot,
            errors::invalid_argument(PROLOGUE_EINVALID_WRITESET_SENDER));
        assert!(Roles::has_diem_root_role(&sender),
            errors::invalid_argument(PROLOGUE_EINVALID_WRITESET_SENDER));
    ...
    fun check_secondary_signers(
        ...
        assert!(vector::length(&secondary_signer_public_key_hashes)
        ...
        while ({
            ...
            assert!(exists_at(secondary_address),
            ...
            assert!(signer_public_key_hash == *&signer_account.authentication_key,
    ...
    spec schema CheckSecondarySignersAbortsIf {
        ...
        aborts_if exists i in 0..num_secondary_signers: !exists_at(
            secondary_signer_addresses[i]) with errors::INVALID_ARGUMENT;
    ...
    fun prologue_common<Token>(
        ...
        assert!(exists_at(transaction_sender),
            errors::invalid_argument(PROLOGUE_EACCOUNT_DNE));

        // [PCA3]: We check whether this account is frozen, if it is no transaction can be sent from it.
        assert!(!AccountFreezing::account_is_frozen(transaction_sender),
        ...
        assert!((txn_sequence_number as u128) < MAX_U64,
        ...
        if (CRSN::has_crsn(transaction_sender)) {
            ...
            assert!(CRSN::check(sender, txn_sequence_number),
            ...
            assert!(txn_sequence_number >= sender_account.sequence_number,
    ...
    spec schema PrologueCommonAbortsIf<Token> {
        ...
        aborts_if hash::sha3_256(txn_public_key) != global<DiemAccount>(transaction_sender)
        ...
        aborts_if !CRSN::has_crsn(transaction_sender) && txn_sequence_number < global<
            DiemAccount>(transaction_sender).sequence_number with errors::INVALID_ARGUMENT;
        /// [PCA12] Covered: L63 (match 3)
        aborts_if !CRSN::has_crsn(transaction_sender) && txn_sequence_number > global<
            DiemAccount>(transaction_sender).sequence_number with errors::INVALID_ARGUMENT;
        /// [PCA13] Covered: L93 (match 14)
        aborts_if CRSN::has_crsn(transaction_sender) && !CRSN::spec_check(
            transaction_sender, txn_sequence_number) with errors::INVALID_ARGUMENT;
    }
    ...
    fun epilogue_common<Token>(
        ...
        assert!((txn_gas_price as u128) * (gas_used as u128) <= MAX_U64,
        ...
        assert!(sender_account.sequence_number < (MAX_U64 as u64),
            errors::limit_exceeded(PROLOGUE_ESEQUENCE_NUMBER_TOO_BIG));

        if (CRSN::has_crsn(sender)) {
            // Make sure the `sender_account`'s CRSN is still valid and record it.
            assert!(CRSN::record(account, txn_sequence_number),
                errors::invalid_argument(PROLOGUE_ESEQ_NONCE_INVALID),);
        } else {
            ...
            assert!(sender_account.sequence_number == txn_sequence_number,
                errors::invalid_argument(PROLOGUE_ESEQUENCE_NUMBER_TOO_NEW));
        };
        ...
        if (transaction_fee_amount > 0) {
            ...
            assert!(transaction_fee_amount <= Diem::value(coin),
    ...
    fun writeset_epilogue(
        ...
        event::emit_event<AdminTransactionEvent>(&mut writeset_events_ref.upgrade_events,
        ...
        assert!(signer::address_of(dr_account) == @DiemRoot,
            errors::invalid_argument(PROLOGUE_EINVALID_WRITESET_SENDER));
        assert!(Roles::has_diem_root_role(dr_account),
            errors::invalid_argument(PROLOGUE_EINVALID_WRITESET_SENDER));
    ...
    spec schema CreateValidatorOperatorAccountAbortsIf {
        ...
        aborts_if ValidatorOperatorConfig::has_validator_operator_config(
            new_account_address) with errors::ALREADY_PUBLISHED;
    }
    ...
    spec schema PreserveKeyRotationCapAbsence {
        ...
        ensures forall addr: address: old(!exists<DiemAccount>(addr) || !spec_has_key_rotation_cap(
                addr)) ==>
    ...
    spec module {
        ...
        invariant forall addr: address where exists_at(addr): spec_holds_delegated_withdraw_capability(
            addr) || spec_holds_own_withdraw_cap(addr);
    }
    ...
    spec schema BalanceNotDecrease<Token> {
        ensures forall addr: address where old(exists<Balance<Token>>(addr)): global<Balance<Token>>(
            addr).coin.value >= old(global<Balance<Token>>(addr).coin.value);
    }
    ...
    spec module {
        ...
        invariant [suspendable] forall addr: address: exists<AccountOperationsCapability>(addr) <==>
        ...
        invariant [suspendable] forall addr: address: exists<VASPDomain::VASPDomains>(addr) <==>
        ...
        invariant forall addr: address: (exists<Balance<XUS>>(addr) || exists<Balance<XDX>>(
                addr)) ==>
        ...
        invariant [suspendable] forall addr: address: exists<DualAttestation::Credential>(addr) <==>
    ...
    spec module {
        ...
        fun spec_has_account_operations_cap(): bool { exists<AccountOperationsCapability>(@DiemRoot) }
    ...
    spec fun prologue_guarantees(sender: signer): bool {
        ...
        DiemTimestamp::is_operating() && exists_at(addr) && !AccountFreezing::account_is_frozen(
            addr)
    }
