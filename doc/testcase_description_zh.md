...
```
10.函数与函数之间留有一个空行
11.一个边界情况，其中'{'的左侧恰好有90个字符长，可以使用边距（如10个字符）来处理原始行以'{'结尾的情况，而无需转到下一行
12.如果多个语句在同一行中，每个语句后都有分号，并在最后一个分号末尾添加注释。
格式化结果应多个带分号的语句各自一行，且使注释位于最后一个分号语句之后
13.对于同一嵌套层次的语句块，保持同样的缩进
14.如果语句块里两行语句之间有一个空行，会保持空行输出
15.如果语句块里两行语句之间有多个空行，会压缩，只输出一个空行
